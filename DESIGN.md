...
> built entirely on the **Telegram ecosystem**, orchestrated with the **Anthropic
> agent-loop pattern** (Planner → Orchestrator → Builder → Tester) and executed on a
> **graph engineering framework** (typed state graph with checkpoints and
> human-in-the-loop interrupts).
>
> You enter through a single **onboarding agent** (Hermes Agent or OpenClaw) that
> interviews you and provisions the whole team, and every agent is grounded in its own
> **knowledge base** — web URLs, Google Drive, and local `.md` / `.pdf` / `.txt` /
> `.csv` files, retrieved via RAG at plan and build time.
...
## 2. Concept Mapping: Raft → Telegram

| TeleRaft concept | Raft equivalent | Telegram primitive |
|---|---|---|
...
## 5. Orchestration: the Anthropic Loop as a State Graph
